...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `점수: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `자른 큐브 개수: ${state.game.cubeCount}`;
}
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				highScoreLblNode.textContent = '최고 기록을 갱신하셨어요!';
			} else {
				highScoreLblNode.textContent = `최고 점수: ${formatNumber(getHighScore())}`;
			}
